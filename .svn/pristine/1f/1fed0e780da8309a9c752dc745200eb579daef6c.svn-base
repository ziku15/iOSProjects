...
@implementation PolicyDetailsTabbarView
...
- (id)initWithFrame:(CGRect)frame with:(id)parentReference
{
    ...
    if (self) {
        ...
        tabScrollview = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 4.5f, frame.size.width, frame.size.height)];
        [tabScrollview setShowsHorizontalScrollIndicator:NO];
        [self addSubview:tabScrollview];
        
        CGFloat x = 10;
        for (int i = 0; i < 4; i ++) {
            PolicyDetailsButton *tempButton = [PolicyDetailsButton buttonWithType:UIButtonTypeCustom];
            [tempButton setTag:i];
            ...
            tempButton.titleLabel.font = [UIFont systemFontOfSize:12.0];
            ...
            tempButton.frame = CGRectMake(x, 0, (tabScrollview.frame.size.width-17)/4, frame.size.height - 10.0f);
        ...
        PolicyDetailsButton *tempButton = [buttonArray objectAtIndex:0];
...
-(IBAction)tabbedAction:(id)sender{
    ...
    PolicyDetailsButton *tempButton = ( PolicyDetailsButton *)sender;
...
-(void)resetScrollviewContent:(int)button_tag{
    for (int i = 0; i < buttonArray.count ; i ++ ) {
        PolicyDetailsButton *button = [buttonArray objectAtIndex:i];
