...
//        NSDateComponents *components = [[NSCalendar currentCalendar] components:NSDayCalendarUnit | NSMonthCalendarUnit | NSYearCalendarUnit fromDate:myDate];
        NSInteger day = [[[CommonHelperClass sharedConstants] getDate:dateStr] integerValue];//[components day];
        NSInteger month = [[[CommonHelperClass sharedConstants] getRemoteMonth:dateStr] integerValue];//[components month];
        NSInteger year = [[[CommonHelperClass sharedConstants] getRemoteYear:dateStr] integerValue];//[components year];
...
- (void)openMovieView:(int)videoTag{
    MediaItem *item = (MediaItem *)[photosArray objectAtIndex:videoTag];
    if (item.mediaType == 2) {
        [self showVideoScreenView:item];
    }
}
...
-(void)getPhoto_Video_Count:(NSArray*)imageArray{

    for(int i=0;i<imageArray.count;i++){
        
        MediaItem *item=(MediaItem*)[imageArray objectAtIndex:i];
        if(item.mediaType==2)
            albumItem.videos++;
            ...
            if (success) {
                ...
                if ([resultDataArray count]>0) {
                  ...
                  //  photosArray=resultDataArray;
                    ...
                    photosArray=galleryView.mediaItemArray;
                    
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [self dismisssView];
                        [self getPhoto_Video_Count:resultDataArray];
                      self.numberOfItemsLabel.text = [NSString stringWithFormat:@"%d photos, %d videos",albumItem.photos,albumItem.videos];
