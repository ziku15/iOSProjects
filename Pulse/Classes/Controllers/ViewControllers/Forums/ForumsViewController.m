...
- (void)viewWillAppear:(BOOL)animated{
   ...
   [self.dataSource removeAllObjects];
    ...
    [tabView resetButtonCondition:selectedTab ];
...
#pragma mark - badge Notification Server Methode
-(void)getBadgeNotification{
...
//    else [self makeServerRequestFor_HashTag_LatestPost];
    ...
    alertMsg = nil;
     ...
     [tabView resetButtonCondition:kAllPosts];
        tabView.alpha = 1.0f;
    [self getBadgeNotification];
       [self tabButtonAction:kAllPosts];
}
                                           ...
                                           completion:^(BOOL success, NSMutableArray *resultDataArray) {
                                               if (self.dataSource != NULL && self.dataSource.count>0) {
                                                   if (success) {
                                                       ...
                                                       if ([selectedType integerValue]==3 && [selectedHashTag isEqualToString:@""]&&resultDataArray.count<=0) {
                                                            noDataLabel.alpha = 1.0f;
                                                       }
...
-(void) loadingPullDownView{
    ...
    }else if (([selectedType integerValue]==3 && ![selectedHashTag isEqualToString:@""])||([selectedType integerValue]==4 && ![selectedHashTag isEqualToString:@""])){
        frame = CGRectMake(0, 65.0f+30.0f, SCREEN_SIZE.width, 40.0f);
    }else{
