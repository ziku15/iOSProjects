...
- (id)initWithFrame:(CGRect)frame withPlaceholderText:(NSString*)ptext{
    ...
    if (self) {
        // Initialization code
        int highlvalue = 48;
        int empty=highlvalue-(int)ptext.length;
        if(empty>0){
...
- (BOOL)searchBarShouldEndEditing:(UISearchBar *)searchBar {
    ...
    int highlvalue = 48;
    int empty=highlvalue-(int)searchBar.placeholder.length;
    if(empty>0){
        for (int i=0; i<empty; i++) {
            searchBar.placeholder=[searchBar.placeholder stringByAppendingString:@" "];
        }
